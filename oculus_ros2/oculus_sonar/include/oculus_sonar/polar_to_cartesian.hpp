...
inline SonarSpecs getM750dSpecs(bool high_freq = true) {
    SonarSpecs specs;
    specs.horizontal_aperture_deg = 70.0f;  // Always 130 degrees regardless of frequency
 ...
 */
inline SonarSpecs getM1200dSpecs(bool high_freq = true) {
    SonarSpecs specs;
    // M1200d: 2.1MHz (high_freq) -> 60° FOV, 1.2MHz (low_freq) -> 130° FOV
    specs.horizontal_aperture_deg = high_freq ? 60.0f : 130.0f;
    specs.vertical_aperture_deg = high_freq ? 12.0f : 20.0f;
    specs.max_beams = 512;
...
public:
    ...
    PolarToCartesianConverter(const std::string& sonar_model = "m750d", int freq_mode = 2)
        ...
        if (sonar_model_ == "m750d") {
            ...
            specs_ = getM1200dSpecs(high_freq);  // FOV depends on frequency
        } else {
    ...
    void generateMappingFromSonarImage(int num_ranges, int num_beams, float max_range,
        ...
        // For M750d and M1200d, use fixed FOV regardless of actual bearings
        if (sonar_model_ == "m750d" || sonar_model_ == "m1200d") {
            // Calculate FOV based on model and frequency
            float fov_deg;
            if (sonar_model_ == "m750d") {
                fov_deg = 70.0f;  // M750d always 130°
            } else {
                // M1200d: 60° for high freq (2.1MHz), 130° for low freq (1.2MHz)
                fov_deg = (freq_mode_ == 2) ? 60.0f : 130.0f;
            }
            ...
            RCLCPP_INFO(rclcpp::get_logger("polar_converter"),
                       "%s: Using fixed %.1f° FOV for %d beams (ignoring actual bearings)",
                       sonar_model_.c_str(), fov_deg, num_beams);
        } else {
            // For other models, use actual bearings from the sonar message
            bearings_.clear();
    ...
    void generateMapping(const liboculus::SimplePingResult<PingT>& ping) {
        ...
        // For Oculus sonars: Use fixed FOV based on model and frequency
        if (sonar_model_ == "m750d") {
            // M750d always has 130° FOV
            float fov_deg = 70.0f;
            float fov_rad = fov_deg * M_PI / 180.0f;

            // Calculate beam spacing based on number of beams
            // beam_spacing = FOV / (num_beams - 1)
            float beam_spacing_rad = fov_rad / (num_beams - 1);
            float beam_spacing_deg = fov_deg / (num_beams - 1);

            // Generate bearings centered at 0
            float start_bearing = -fov_rad / 2.0f;
            ...
            RCLCPP_INFO(rclcpp::get_logger("polar_converter"),
                       "M750d: %d beams across %.1f° FOV, beam spacing: %.4f°",
                       num_beams, fov_deg, beam_spacing_deg);

            // Log if this is likely downsampled
            if (num_beams == 256) {
                RCLCPP_INFO(rclcpp::get_logger("polar_converter"),
                           "Using 256 beams (downsampled mode) with %.3f° spacing",
                           beam_spacing_deg);
            } else if (num_beams == 512) {
                RCLCPP_INFO(rclcpp::get_logger("polar_converter"),
                           "Using 512 beams (full resolution) with %.3f° spacing",
                           beam_spacing_deg);
            }
        } else if (sonar_model_ == "m1200d") {
            // M1200d FOV depends on frequency mode
            // freq_mode_ == 2 (2.1MHz high freq) -> 60° FOV
            // freq_mode_ == 1 (1.2MHz low freq) -> 130° FOV
            float fov_deg = (freq_mode_ == 2) ? 60.0f : 70.0f;
            float fov_rad = fov_deg * M_PI / 180.0f;
            ...
            RCLCPP_INFO(rclcpp::get_logger("polar_converter"),
                       "M1200d (%.1fMHz): %d beams across %.1f° FOV, beam spacing: %.4f°",
                       (freq_mode_ == 2) ? 2.1f : 1.2f, num_beams, fov_deg, beam_spacing_deg);
        } else {
            // For other models, use actual bearings from hardware
            for (int i = 0; i < num_beams; ++i) {
        ...
        if (!bearings_.empty()) {
            ...
            // Calculate actual beam spacing
            float bearing_spacing_deg = fov_deg / (num_beams - 1);

            // Expected spacing based on number of beams and FOV
            float expected_spacing = (sonar_model_ == "m750d") ?
                ((num_beams == 512) ? 0.25f : 0.5f) :  // M750d: 0.25° for 512 beams, 0.5° for 256 beams
                0.25f;  // Other models default to 0.25°

            RCLCPP_INFO(rclcpp::get_logger("polar_converter"),
                        "FOV: %.2f°, beams: %d, spacing: %.4f° (expected %.2f° for %s)",
                        fov_deg, num_beams, bearing_spacing_deg, expected_spacing,
                        (num_beams == 256) ? "256 beams (downsampled)" : "512 beams (full resolution)");
            RCLCPP_INFO(rclcpp::get_logger("polar_converter"),
