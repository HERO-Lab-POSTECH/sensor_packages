...
void OculusFanImager::init() {
  ...
  this->declare_parameter<int>("colormap_type", cv::COLORMAP_JET);  // OpenCV colormap
...
void OculusFanImager::sonarImageCallback(const marine_acoustic_msgs::msg::SonarImage::SharedPtr msg) {
  try {
    ...
    // Generate mapping with actual bearings from message
    polar_converter_->generateMappingFromSonarImage(num_ranges, num_beams, max_range, 
                                                    msg->azimuth_angles);
...
rcl_interfaces::msg::SetParametersResult OculusFanImager::parameterCallback(
  ...
  for (const auto& param : parameters) {
    ...
    else if (param.get_name() == "colormap_type") {
      colormap_type_ = param.as_int();
      RCLCPP_INFO(this->get_logger(), "Colormap type changed to %d", colormap_type_);
    }
    else if (param.get_name() == "sonar_model") {
      std::string new_model = param.as_string();
      if (new_model != sonar_model_) {
        sonar_model_ = new_model;
        // Recreate converter with new model
        polar_converter_ = std::make_unique<PolarToCartesianConverter>(sonar_model_, freq_mode_);
  ...
  if (frame_count_ > 0) {
    ...
    if (elapsed_sec < 1.0) {
      RCLCPP_INFO(this->get_logger(), 
                  "Fan Imager Stats: Processing %d frames, last update %.2f sec ago",
